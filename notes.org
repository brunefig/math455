...
:END:
***** Cloze
If $\tau$ is {{c1::the discrete topology}} and
$\forall n \in \mathbb{N}$ : $X_n$ := {{c1::$\bigl(\{0,1\},\tau\bigr)$}},
then $\prod_{n \in \mathbb{N}} X_n$ is not compact under {{c2::the discrete topology}}
because {{c3::it is an open cover of itself but has no finite subcover}}.
*** countable Tychonoff
:PROPERTIES:
:ANKI_NOTE_TYPE: Cloze
:END:
